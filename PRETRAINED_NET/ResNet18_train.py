...
data_dir = 'project/FLAV/deepCovid/dataset'
...
def train_model(model, criterion, optimizer, scheduler, batch_szie, num_epochs):
    print("I'm training for", num_epochs, " epochs\n")
    since = time.time()
...
if __name__ == "__main__":
    ...
    now = datetime.now()
    print("\ntest")
    name = str(args.epochs)+str(now)
    print(name)

    torch.save(model_conv, './covid_resnet18_'+name+'.pt')
    print("Model saved as covid_resnet18_"+name+".pt")
